...
import { useFullscreen } from '@/hooks/useFullScreen';
import { useChartData } from '@/hooks/useChartData';
import { useTrendAnalysis } from '@/hooks/useTrendAnalysis'; // Import our simplified hook
import ExportService from '@/services/ExportService';
import { getLineColor } from '@/utils/formatter';
import { ChartActionsMenu } from './ChartActionsMenu';
import { LineChartComponent } from './LineChart'; // Import our simplified component
import { cn } from '@/lib/utils';
...
const EngChart = ({ selectedTags, equipmentData, dateRange: propDateRange }) => {
  ...
  // Use our simplified trend analysis hook
  const { analyzeAllTags, trendAnalysis, isAnalyzing } = useTrendAnalysis(
    dataToUse,
    selectedTags,
    productionDayStart,
    productionDayEnd
  ...
  // Show trend analysis
  const handleShowTrendAnalysis = async () => {
    if (!trendAnalysis) {
      await analyzeAllTags();
    }
    setShowTrendReport(true);
  };
      ...
      <div className={cn("flex justify-between items-center mb-[1rem] px-[1rem]", isFullscreen && 'px-[2rem] mb-[2rem]')}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={handleShowTrendAnalysis}
            disabled={isAnalyzing}
          >
            <FileText size={16} />
            {isAnalyzing ? "Analyzing..." : "View Trends"}
          </button>
